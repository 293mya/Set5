...
- `hyperloglog.cpp` — базовая реализация (Этап 1–2) + генерация данных для анализа (Этап 3).
  - выводит: `hll_results_all_streams.csv`, `hll_stream_*.csv`
- `hyperloglog_stage4.cpp` — улучшение для Этапа 4 (packed-регистры 6 бит) + генерация данных для сравнения.
  - выводит: `stage4_hll_results_all_streams.csv`, `stage4_hll_stream_*.csv`
- `analyze_hll.py` — анализ baseline: строит `graph1.png`, `graph2.png`, печатает mean/std относительной ошибки.
- `analyze_hll_stage4.py` — анализ Этапа 4: строит `graph1_stage4.png`, `graph2_stage4.png`, печатает mean/std для baseline vs packed6.
- `graph1.png`, `graph2.png` — графики для Этапа 3.
- `graph1_stage4.png`, `graph2_stage4.png` — графики для Этапа 4.
...
Я сравниваю **практические результаты** работы моего HyperLogLog с **теоретическими оценками** из задания.
	...
	B=10 (m=1024) — компромисс память/точность, ожидаемый разброс ~ 1.04/sqrt(1024) ≈ 3.25%.
---
...
![Graph 1 (baseline)](graph1.png)
![Graph 2 (baseline)](graph2.png)
...
![Graph 1 (stage4)](graph1_stage4.png)
![Graph 2 (stage4)](graph2_stage4.png)